...
class ViewAllFeaturedProperties extends StatefulWidget {
  final String name;
  const ViewAllFeaturedProperties({super.key, required this.name});

  @override
  State<ViewAllFeaturedProperties> createState() =>
      _ViewAllFeaturedPropertiesState();
}

class _ViewAllFeaturedPropertiesState extends State<ViewAllFeaturedProperties> {
  bool _isInit = true;
  int currentPage = 1;
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      _data = Provider.of<PropertiesProvider>(context, listen: false)
          .getFeaturedProperties(page: currentPage, isFromHome: false);
      _isInit = false;
    }
                ...
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return Consumer<PropertiesProvider>(
                    builder: (context, value, _) {
                      if (value.featuredProp.isEmpty) {
                        return const Center(child: Text("No Properties"));
                      }
                      sortBy == 0 ? value.sortByPopular(true, false) : null;
                      sortBy == 1 ? value.sortByNewest(true, false) : null;
                      sortBy == 2 ? value.sortByPriceLowest(true, false) : null;
                      sortBy == 3
                          ? value.sortByPriceHighest(true, false)
                          : null;
                      return Column(
                        children: [
                          SizedBox(
                          ...
                          ListView.builder(
                              ...
                              itemCount: value.featuredProp.length,
                              itemBuilder: (ctx, index) {
                                return ListViewWidget(
                                  id: value.featuredProp[index].id,
                                  propertyName: value.featuredProp[index].name,
                                  price: value.featuredProp[index].price,
                                  bedroom:
                                      value.featuredProp[index].details.bedroom,
                                  bathroom: value
                                      .featuredProp[index].details.bathroom,
                                  area: value.featuredProp[index].details.area,
                                  address:
                                      value.featuredProp[index].address.city,
                                  type: value.featuredProp[index].propertyType,
                                  imgUrl:
                                      value.featuredProp[index].images[0].url,
                                );
                              }),
                        ],
                      );
