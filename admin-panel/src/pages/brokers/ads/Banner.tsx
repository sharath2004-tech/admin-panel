...
const Banner: React.FC = () => {
  ...
  const bannerData = useFetchData(
    ...
    `ads/broker/get-all-ads/${user?.broker}?page=${currentPage}&perPage=${perPage}`,
    headers
  );
  return (
    <div className="p-3 flex flex-col items-start space-y-3 w-full bg-white dark:bg-secondary-dark-bg my-transition">
      <HeaderContainer headerTitle="Banner">
        <Button onClick={() => navigate("/ads/post-ad")}>Create Ad</Button>
      </HeaderContainer>
