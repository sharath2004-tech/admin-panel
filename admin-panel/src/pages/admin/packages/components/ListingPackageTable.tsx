...
import { BrokerPackage } from "@/types/Package";
...
const ListingPackageTable: React.FC<TableProps> = ({
  data,
  setCurrentPage,
  setPerPage,
}) => {
  ...
  const AgentTableColumns: ColumnDef<BrokerPackage>[] = [
    {
    ...
    },
    {
      ...
      header: () => {
        return <Button variant="ghost">Package Price</Button>;
      },
      cell: ({ row }) => {
        return (
          <h4 className="font-medium capitalize dark:text-gray-300">
            {row?.original?.package?.price} USD
          </h4>
        );
      },
    },
        ...
        return (
          <DropdownMenu>
            ...
            >
              ...
              <DropdownMenuItem
                onClick={() =>
                  navigate(`/listing-packages/detail/${row.original._id}`)
                }
...
export default ListingPackageTable;
