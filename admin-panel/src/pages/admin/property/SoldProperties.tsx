...
const SoldProperties: React.FC = () => {
  ...
  const soldProperties = useFetchData(
    ["SoldPropertyDataApi", currentPage, perPage, stateChange, filterType],
    `property/sold/admin/all?page=${currentPage}&perPage=${perPage}&type=${filterType}`,
    headers
  );
  return (
    <div className="p-3 flex flex-col items-start space-y-3 w-full bg-white dark:bg-secondary-dark-bg my-transition">
      ...
      <div className="w-full">
        {soldProperties.isFetched && soldProperties.isSuccess ? (
          <div className="w-full">
            <BrokersPropertyTable
              data={soldProperties?.data?.pagination}
      ...
      {soldProperties.isFetched && soldProperties.isError && (
