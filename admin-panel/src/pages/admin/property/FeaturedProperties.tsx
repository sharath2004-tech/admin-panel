...
const FeaturedProperties: React.FC = () => {
  ...
  const featuredProperties = useFetchData(
    ["FeaturedPropertyDataApi", currentPage, perPage, stateChange, filterType],
    `property/featured/admin/all?page=${currentPage}&perPage=${perPage}&type=${filterType}`,
    headers
  );
  return (
    <div className="p-3 flex flex-col items-start space-y-3 w-full bg-white dark:bg-secondary-dark-bg my-transition">
      <HeaderContainer headerTitle="Featured Properties">
        <Select
      ...
      <div className="w-full">
        {featuredProperties.isFetched && featuredProperties.isSuccess ? (
          <div className="w-full">
            <BrokersPropertyTable
              data={featuredProperties?.data?.pagination}
      ...
      {featuredProperties.isFetched && featuredProperties.isError && (
